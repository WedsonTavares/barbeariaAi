...
import { avisosAction } from "./avisos-action";

type Avisos = Awaited<ReturnType<typeof avisosAction>>;

const INTERVALO_MS = 30_000;

/**
 * Avisos ao vivo no topo do painel: conversas com mensagem sem ler e
 ...
 */
export function AvisosAoVivo({ inicial }: { inicial: number }) {
  const [dados, setDados] = useState<Avisos | null>(null);
  ...
  const avisos = dados?.avisos ?? inicial;
  const conversas = dados?.conversas ?? 0;
